document.addEventListener('DOMContentLoaded', function() {
    ...
    const urlp = `https://graph.microsoft.com/v1.0/me/drive/sharedWithMe`;
    socket.emit('haConnectionC', urlp);
    ...
    function createRow(type, name,  id, driveid, fold) {
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td>${type}</td>
            <td class="nombre" data-id="${id}" data-driveId="${driveid}" data-name="${name}" data-fold="${fold}">${name}</td>
            <td><button class="descargar" data-id="${id}" data-driveId="${driveid}" data-name="${name}" data-fold="${fold}">Descargar</button></td>
        `;
        return tr;
    }
    ...
    function addFolderClickEvents() {
        ...
        carpetas.forEach(carpeta => {
            carpeta.addEventListener('click', function() {
                ...
                const fold = this.dataset.fold;
                const driveId = this.dataset.driveid;              
                socket.emit('mostrarContenidoCarpetaC', { id, driveId, name, fold });
    ...
    socket.on('carpetas', (data) => {
        ...
        contenido.folders.forEach(folder => {
            const row = createRow('Carpeta', folder.name, folder.id, folder.driveId, nameFolder);
        ...
        contenido.files.forEach(file => {
            const row = createRow('Archivo', file.name, file.id, file.driveId, nameFolder);
    ...
    rutaLista.addEventListener('click', (event) => {
        ...
        const driveId = target.dataset.driveid;
        socket.emit('mostrarContenidoCarpetaC', { id, driveId, name, fold });
        let eliminar = false;
